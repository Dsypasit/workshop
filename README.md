...
1. set ค่า `WORKFLOW_TOKEN` ใน Github ของทีม
   1. กลับไปที่ Repository workshop ของเรา ที่เรา fork มา (e.g. https://github.com/dsypasit/workshop)
...
1. แก้ไข CI/CD ให้เป็นชื่อ repository ของทีม
   ...
   1. เปิดโปรเจคของเราด้วย VSCode แล้ว find and replace `dsypasit` ให้เป็นชื่อ github account ของคุณคนที่ fork มา (dsypasit MUST be lowercase)
...
6. setup gitops สำหรับ development env
   - กด `+ New App` แล้วใส่ข้อมูลดังนี้
     ...
     - ✅ PRUNE RESOURCES
     - Repository URL: `https://github.com/dsypasit/workshop`
     - Revision: `main`
     - Path: `infra/gitops/dev`
     - Cluster URL: `https://kubernetes.default.svc`
     - กด `Create` มุมบนซ้าย

note: ตรวจสอบให้แน่ใจว่าที่ [https://github.com/dsypasit?tab=packages](https://github.com) ที่ workshop เป็น public (ไม่มีคำว่า private แสดงอยู่)
